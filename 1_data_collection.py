...
Fetches stock data from yfinance and scrapes news headlines
"""
...
def scrape_news_headlines_yahoo(ticker, start_date, end_date):
    """
    Scrape news headlines for a stock ticker using Yahoo Finance RSS feed
    """
    all_headlines = {}
...
def scrape_news_headlines_alternative(ticker, start_date, end_date):
    """
    Alternative method: Scrape from financial news websites
    This is a fallback if RSS feed doesn't work
    """
    all_headlines = {}
    
    # Try to get news from various sources
    # For now, we'll use a simple approach with Yahoo Finance news page
    try:
...
def scrape_news_headlines(ticker, start_date, end_date):
    """
    Main function to scrape news headlines
    ...
    # Fill in missing dates with empty lists (for sentiment analysis)
...
if __name__ == "__main__":
    ...
    # Stock data: fetch from 2015 for training (or user-specified start date)
    STOCK_START_DATE = "2015-01-01"
    if len(sys.argv) > 2:
        STOCK_START_DATE = sys.argv[2]
    
    # News: only fetch from past 30 days
    END_DATE = datetime.now().strftime("%Y-%m-%d")
